...
class CharsiuPreprocessor:
    ...
    def long_audio_process(self, audio, sr=16000, segment_size_sec=10):
        ...
        features = self.processor(features, sampling_rate=16000,return_tensors='pt').input_values.squeeze()
        segment_size = segment_size_sec * sr
        segments = list(torch.split(features, segment_size))
        return segments
...
#        self.punctuation = set('.,!?')
    ...
    def align_words_corrupted(self, preds, phones, words):


        
        phones = [[re.sub(r'\d','',p) for p in list(phone)] for phone in phones]
        phones2word = {}
        for word, phone in zip(words, phones):
            phones2word[''.join(phone)] = word
        word_alignment = []
        word = []
        start_time = preds[0][0] if preds[0][2] != "[SIL]" else preds[1][0] 
        for idx, frame in enumerate(preds):
            phone = frame[2]
            end_time = frame[1]
            if frame[2] == "[SIL]":
                word_alignment.append(frame)
                start_time=end_time
                continue
            ...
            # print(phones[0])
            if word in phones:
                word_alignment.append((start_time, end_time, phones2word[''.join(word)]))
                start_time = end_time
                word = []
...
class CharsiuPreprocessor_nb(CharsiuPreprocessor):
...
#        self.punctuation = set('.,!?')
        self.punctuation = set()     
        self.word_2_phones =  self.norwegian_dict()

    def norwegian_dict(self):
        norw_dict = {}
        with open("/home/theokouz/src/charsiu/norwegian_lang/lexicon_nov22.txt") as fd:
            lines = [ln.rstrip().split() for ln in fd]
        for word, *phones in lines:
            norw_dict[word] = phones
        return norw_dict
 
    def g2p_norwegian(self, sent):
        """
        ...
        for word in words[:-1]:
            phone_sequense+=self.word_2_phones[word]
            phone_sequense+=[" "]
        phone_sequense+=self.word_2_phones[words[-1]]
        return phone_sequense
...
if __name__ == '__main__':
    ...
    Testing functions
    '''    
    processor = CharsiuPreprocessor_nb()
    phones, words = processor.get_phones_and_words("ved ei stor elv vi vasker oss i elva når vi skal dra et annet sted tar vi båten")
